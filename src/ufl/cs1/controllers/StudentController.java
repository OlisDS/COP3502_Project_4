...
public final class StudentController implements DefenderController
{
    ...
    public int stalker(int ghostID)
	{
		...
		for(Defender d : enemies){
			if(d.getLocation().getPathDistance(attackerLocation) >= 20)
				ghostInRange = false;
		}
		...
		for (Defender d : enemies){
			if (d == ghost)
				continue; //excluding the kamikaze ghost
		...
		if((attacker.getLocation() == powerPills && safeToRushTheAttacker) || (attacker.getLocation() != powerPills && ghostInRange))
			return ghost.getNextDir(attackerLocation, true);
		else if(attackerLocation == powerPills && !safeToRushTheAttacker)
